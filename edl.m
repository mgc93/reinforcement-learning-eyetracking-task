...
yLength = globalRect(4)/4;
xLength = globalRect(3)/4;

xStart = [xLength/2, midX+xLength/2, midX-xLength/2, midX-xLength/2];
yStart = [midY-yLength/2, midY-yLength/2, yLength/2, midY+yLength/2];
...
% AOI with margins
rectAOI(1,:) = calcAOI(rectC(1,:),ScreenX,ScreenY);
rectAOI(2,:) = calcAOI(rectC(2,:),ScreenX,ScreenY);
rectAOI(3,:) = calcAOI(rectC(3,:),ScreenX,ScreenY);
rectAOI(4,:) = calcAOI(rectC(4,:),ScreenX,ScreenY);
...
xposN = [midX, midX];
yposN = [yStart(3)+yLength/2-yLength/7, yStart(4)+yLength/2-yLength/7];
...
if trackEye == 1
    ...
    edfNTrials = 105;
    %create edf file
    edfFileName1 = ['ND_P1_' num2str(sID)]; % CHANGE TO INCLUDE SUBJ. NUMBER, BUT LESS THAN 8 CHARACTERS. %['SelfControl' num2str(sID)];
    ...
    % automatically)
    status = Eyelink('openfile', edfFileName1);
    % if something goes wrong with creating the EDF, shut it down.
    if status~=0
        fprintf('Cannot create EDF file ''%s'' ', edfFileName1);
        Eyelink('Shutdown');
        Screen('CloseAll');
        return;
    end
...
for i = 1:size(R,1)
    tex{i,1} = Screen('MakeTexture',window,R(i,1).img);
end

imgTrial1 = zeros(size(trialMat1,1),2);
for i = 1:size(trialMat1,1)
    imgTrial1(i,1) = str2num(R(trialMat1(i,1),1).id);
    imgTrial1(i,2) = str2num(R(trialMat1(i,2),1).id);
end
...
trialN1 = size(isi_t2_1,1);
eyeDataFeedback = [];
eyeDataChoice = [];
t1_eval = zeros(1,trialN1);
t1_write = zeros(1,trialN1);
for trial = 1:3 %trialN1
        ...
        if trackEye == 1
            ...
            % side of the screen.
            Eyelink('Message', '!V IAREA RECTANGLE %d %d %d %d %d %s', 1, rectAOI(1,1),rectAOI(1,2),rectAOI(1,3),rectAOI(1,4),'left');
            Eyelink('Message', '!V IAREA RECTANGLE %d %d %d %d %d %s', 2, rectAOI(2,1),rectAOI(2,2),rectAOI(2,3),rectAOI(2,4),'right');
            ...
            Screen('FrameRect',window,frameColor,rectC(1,:),2);
            Screen('FrameRect',window,frameColor,rectC(2,:),2);
                ...
                if status >= 0
                    ...
                    if eyeX ~= el.MISSING_DATA && eyeY ~= el.MISSING_DATA && evt.pa(eye_used+1)>0
                        ...
                        elseif eyeY >= rectAOI(2,2) && eyeY <= rectAOI(2,4)
                            if eyeX >= rectAOI(2,1) && eyeX <= rectAOI(2,3)
                                lookLeft = 0;
                            end
                        end
                        ...
                        % only need one.
                        if lookLeft ~= prevlookLeft && fixEnd ~= 0 && fixStart ~= -1
...
%                     end
        ...
        if trackEye == 1
            ...
            % ROI. The examples below are two ROIs for the left and right
            % side of the screen.
            Eyelink('Message', '!V IAREA RECTANGLE %d %d %d %d %d %s', 3, rectAOI(3,1),rectAOI(3,2),rectAOI(3,3),rectAOI(3,4),'top');
            Eyelink('Message', '!V IAREA RECTANGLE %d %d %d %d %d %s', 4, rectAOI(4,1),rectAOI(4,2),rectAOI(4,3),rectAOI(4,4),'bottom');
          ...
          if(trial>2)  
            ...
            if(~isnan(ansBox1(trial,1)))
                ...
                if(~isnan(ansBox1(trial-1,1)))
                    ...
                    Screen('FrameRect',window,frameColor,rectC(3,:),2);
                    Screen('FrameRect',window,frameColor,rectC(4,:),2)
            ...
            while(GetSecs-trialOnset<2)
                ...
                if status >= 0
                    ...
                    if eyeX ~= el.MISSING_DATA && eyeY ~= el.MISSING_DATA && evt.pa(eye_used+1)>0
                        ...
                        % See which ROI they're looking at - if they're
                        % outside of both roi's, it's counted as a -1
                        if eyeY >= rectAOI(3,2) && eyeY <= rectAOI(3,4)
                            if eyeX >= rectAOI(3,1) && eyeX <= rectAOI(3,3)
                                lookTop = 1;
                            end
                        elseif eyeY >= rectAOI(4,2) && eyeY <= rectAOI(4,4)
...
totalMoney1 = 0.05*(totalPay1 - thresholdPay1);
...
answerMat1 = [rt1, ansBox1, ansSquare1, ansPay1, ansPayNow1, ansPayLater1,...
    ...
    payTrialMat1, imgTrial1, trialNumber, blockNumber(:), partNumber1];
...
DrawFormattedText(window,['You can take a short break before part 2 starts','\n',...
    'When you are ready press any key to continue.'],'center','center',textColor,[],[],[],1.5);
Screen('Flip',window);
KbWait([], 2);
KbWait([], 1);
...
if trackEye == 1
    ...
    edfNTrials = 105;
    %create edf file
    edfFileName2 = ['ND_P2_' num2str(sID)]; % CHANGE TO INCLUDE SUBJ. NUMBER, BUT LESS THAN 8 CHARACTERS. %['SelfControl' num2str(sID)];
    ...
    % open file to record data to (which is basically done
    % automatically)
    status = Eyelink('openfile', edfFileName2);
    % if something goes wrong with creating the EDF, shut it down.
    if status~=0
        fprintf('Cannot create EDF file ''%s'' ', edfFileName2);
        Eyelink('Shutdown');
        Screen('CloseAll');
        return;
    end
...
imgTrial2 = zeros(size(trialMat2,1),2);
for i = 1:size(trialMat1,1)
    imgTrial2(i,1) = str2num(R(trialMat2(i,1),1).id);
    imgTrial2(i,2) = str2num(R(trialMat2(i,2),1).id);
end
...
trialN2 = size(isi_t2_2,1);
t2_eval = zeros(1,trialN2);
t2_write = zeros(1,trialN2);
for trial = 1:3 %trialN2
        ...
        if trackEye == 1
            ...
            % in the edf of when a trial starts, and what part of the study
            % it belongs to if you have several parts.
            Eyelink('Message', 'SYNCTIME_part2');
            % This is just telling us that the trial is valid (useful for
            % if you recallibrate during a trial, in which case this will
            % later be switched to 0 so you know to throw the trial out)
            Eyelink('Message', '!V TRIAL_VAR VALID_TRIAL %d', 1);
            ...
            while (GetSecs-trialOnset<=3)
                ...
                if status >= 0
                    ...
                    if eyeX ~= el.MISSING_DATA && eyeY ~= el.MISSING_DATA && evt.pa(eye_used+1)>0
                        ...
                        if lookLeft ~= prevlookLeft && fixEnd ~= 0  && fixStart ~= -1
                            ...
                            % the roi they switched to
                            fprintf(eyeTrackDataFile, eyeLabel, ...
                                'c','2',num2str(sID), num2str(trial), num2str(prevlookLeft), num2str(fixStart), num2str(fixEnd));
                            eyeDataChoice = [eyeDataChoice; 2, sID, trial, prevlookLeft, fixStart, fixEnd];
                ...
                if(keyIsDown~=1)
                    ...
                    if(keyCode(leftKey))
                        ...
                        %                     Screen('FrameRect',window,frameColor,rectC(boxID,:),6);
                        %                     Screen('Flip',window);
                        rt2(trial,1)   = GetSecs-trialOnset;
                        
                        
                        if trackEye == 1
                            Eyelink('Message', 'choiceMade_train2');
                            % If they made their choice while
                            % staring at an ROI, put down RT
                            % instead of "fixend" - also it says
                            % "prevLookLeft" here because we want
                ...
                if status >= 0
                    ...
                    if eyeX ~= el.MISSING_DATA && eyeY ~= el.MISSING_DATA && evt.pa(eye_used+1)>0
                        ...
                        if lookTop ~= prevlookTop && fixEnd ~= 0
                            ...
                            % the roi they switched to
                            fprintf(eyeTrackDataFile, eyeLabel, ...
                                'f','2',num2str(sID), num2str(trial), num2str(prevlookTop), num2str(fixStart), num2str(fixEnd));
                            eyeDataFeedback = [eyeDataFeedback; 2, sID, trial, prevlookTop, fixStart, fixEnd];
...
totalMoney2 = 0.05*(totalPay2 - thresholdPay2);
...
answerMat2 = [rt2, ansBox2, ansSquare2, ansPay2, ansPayNow2, ansPayLater2,...
    ...
    payTrialMat2, imgTrial2, trialNumber, blockNumber(:), partNumber2];
...
while(secretEnd==0)
    ...
    DrawFormattedText(window,['Your total points for the experiment: ', num2str(totalPay1+totalPay2),'\n',...
        'Your total earnings ($) ','\n',...
        'for the experiment (including the show-up bonus): ', num2str(totalMoney)],'center','center',textColor,[],[],[],1.5);
    Screen('Flip',window);
...
dataFile = fullfile('completeData',[datestr(now,'mm-dd-yyyy'),'-S',num2str(sID),'-C',num2str(sCondition),'-BEH','.mat']);
...
% eyetracking data
eyeTrials1 = edfImport(['ND_P1_',num2str(sID)]);
eyeTrialsEvents1 = edfExtractInterestingEvents(eyeTrials1);
eyeTrialsVar1 = edfExtractVariables(eyeTrials1);

eyeTrials2 = edfImport(['ND_P2_',num2str(sID)]);
eyeTrialsEvents2 = edfExtractInterestingEvents(eyeTrials2);
eyeTrialsVar2 = edfExtractVariables(eyeTrials2);

dataFileEye = fullfile('completeDataEye',[datestr(now,'mm-dd-yyyy'),'-S',num2str(sID),'-C',num2str(sCondition),'-EYE','.mat']);
save(dataFileEye,'eyeTrials1','eyeTrialsEvents1','eyeTrialsVar1','eyeTrials2','eyeTrialsEvents2','eyeTrialsVar2');
...
display(totalMoney);
    ...
    matlabfile = ['data_' num2str(sID) '.mat'];
    save(matlabfile);
